...
const CreatePollForm = ({ onPollCreated }) => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200/60 mb-10 transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input 
            ...
            className="w-full px-5 py-3.5 bg-white/50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400 font-medium"
          />
        </div>
            ...
            {options.map((opt, idx) => (
              <div key={idx} className="flex gap-3 group animate-fade-in-up">
                <input 
                  ...
                  className="flex-1 px-5 py-3.5 bg-white/50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400 font-medium"
                />
                <button 
                  type="button"
        ...
        <div className="pt-2 border-t border-slate-100">
          ...
          <select 
            ...
            className="w-full px-5 py-3.5 bg-white/50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium appearance-none cursor-pointer"
          >
        ...
        <button 
          type="submit" 
          disabled={loading}
          className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white py-4 rounded-xl font-bold shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:-translate-y-0.5 transition-all duration-200 flex justify-center items-center mt-2 disabled:opacity-70 disabled:hover:translate-y-0"
        >
